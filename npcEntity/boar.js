class Boar {
    constructor(game, x, y, path) {
        ...
        this.visualRadius = 450;

        this.initialPoint = { x, y };
        this.spritesheet = ASSET_MANAGER.getAsset("./sprites/enemy/boarWalk.png");
        this.spritesheet2 = ASSET_MANAGER.getAsset("./sprites/enemy/boarThrust.png");
        this.targetID = 0;
  ...
  if(this.state==0 ){
    this.animations[this.state][0].drawFrame(this.game.clockTick, ctx, this.x - 60 - this.game.camera.x, this.y - 40 - this.game.camera.y, 2);

  }
  else if( this.state==1 && this.faceleft ==true){
    ctx.save();
ctx.scale(-1, 1);
this.animations[1][0].drawFrame(this.game.clockTick, ctx, -this.x-70 + this.game.camera.x, this.y - 40 - this.game.camera.y, 2);
ctx.restore();
    ...
    this.animations[this.state][0].drawFrame(this.game.clockTick, ctx, this.x -60 - this.game.camera.x, this.y - 40 - this.game.camera.y, 2);
        ...
        ctx.save();
        ctx.scale(-1, 1);
        this.animations[0][0].drawFrame(this.game.clockTick, ctx, -this.x -70 + this.game.camera.x, this.y - 40 - this.game.camera.y, 2);
        ctx.restore();
    }
